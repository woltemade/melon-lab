...
const delegateTakeOrder = async (
  ...
  { fundAddress, exchangeAddress, giveAsset, identifier, signature },
): Promise<Order> => {
  ...
  const orderExpired = await fundContract.instance.orderExpired({}, [
    exchangeAddress,
    giveAsset,
  ]);
  ensure(
  ...
  const cancelLog = await callOnExchange(environment, {
    ...
    exchangeAddress,
    method,
    orderAddresses: ['0x0', '0x0', '0x0', '0x0', '0x0'],
