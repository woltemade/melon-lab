...
export const getTokenPrecisionByAddress = address => {
  ...
  if (address === addressList.statusToken)
    return constants.STATUSTOKEN_PRECISION;
  if (address === addressList.aventcoinToken)
    return constants.AVENTCOINTOKEN_PRECISION;
  if (address === addressList.bancorToken)
    return constants.BANCORTOKEN_PRECISION;
  if (address === addressList.aragonToken)
    return constants.ARAGONTOKEN_PRECISION;
  if (address === addressList.basicAttentionToken)
    return constants.BASICATTENTIONTOKEN_PRECISION;
  if (address === addressList.dogecoinToken)
    return constants.DOGECOINTOKEN_PRECISION;
  if (address === addressList.digixDaoToken)
    return constants.DIGIXDAOTOKEN_PRECISION;
  if (address === addressList.litecoinToken)
    return constants.LITECOINTOKEN_PRECISION;
  if (address === addressList.digixGoldToken)
    return constants.DIGIXGOLDTOKEN_PRECISION;
  if (address === addressList.etherClassicToken)
    return constants.ETHERCLASSICTOKEN_PRECISION;
  if (address === addressList.makerDaoToken)
    return constants.MAKERDAOTOKEN_PRECISION;
  if (address === addressList.golemNetworkToken)
    return constants.GOLEMNETWORKTOKEN_PRECISION;
  if (address === addressList.rippleToken)
    return constants.RIPPLETOKEN_PRECISION;
  if (address === addressList.singularDtvToken)
    return constants.SINGULARDTVTOKEN_PRECISION;
  if (address === addressList.zeroXToken) return constants.ZEROXTOKEN_PRECISION;
  if (address === addressList.gnosisToken)
    return constants.GNOSISTOKEN_PRECISION;
  if (address === addressList.iconomiToken)
    return constants.ICONOMITOKEN_PRECISION;
