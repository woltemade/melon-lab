...
const createVault = async (portfolioName, from = setup.web3.accounts[0]) => {
  ...
  const receipt = await versionContract.createVault(
    portfolioName,
    portfolioSymbol,
    portfolioDecimals,
    universe,
  ...
  const vaultAddedMessage = receipt.logs.find(
    log => log.event === "VaultAdded",
  );
