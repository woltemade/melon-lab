...
const updateFilter = subscriptions => {
  ...
  if (addresses.size || hashes.size) {
    web3Filter = setup.web3.eth.filter({
      address: [...addresses.values()],
      topics: [...hashes.values()].map(h => [h]),
    });

    web3Filter.watch(async (err, event) => {
      if (err) onError(err);
      const config = eventDefinitions.find(e => e.hash === event.topics[0]);
      if (!config) onError(new Error(`No event config for ${event.topics[0]}`));
  ...
  on(...filters) {
    ...
    tracker.times = n =>
      new Promise(resolve => {
        ...
        const hoistedWaiting = {
          filters,
          callback: (name, args) => {
            i += 1;
            if (i >= n) {
              subscriptions.delete(hoistedWaiting);
              resolve(name, args);
            }
          },
        };

        subscriptions.add(hoistedWaiting);
      });
