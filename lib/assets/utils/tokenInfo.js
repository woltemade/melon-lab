const tokenInfo = {
  kovan: [
    {
      address: "0xD99aED09A65ee8377A2fFad8ED5407785c530869",
      name: "Aragon Network Token",
      symbol: "ANT-T",
      decimals: 18,
      url: "https://aragon.one/",
      githubHint: "",
    },
    /* Currently not in Universe (at: 0xf9d242dab5047851caac47bf5f05c2786d6f64a9)
    {
      address: '0x20A0E0dECE9FB6C690CdB439db2B385d271BdA3f',
      name: 'AventCoin',
      symbol: 'AVT-T',
      decimals: 18,
      url: 'https://aventus.io/',
      githubHint: '',
    },
    */
    {
      address: "0x764ae227ad6a6e143546772169265d76aA9337c6",
      name: "Bancor Network Token",
      symbol: "BNT-T",
      decimals: 18,
      url: "https://www.bancor.network/",
      githubHint: "",
    },
    {
      address: "0xFEE1D0Dc0b5b6F2F20D8e9f7E95e9E367E4a61A7",
      name: "Basic Attention Token",
      symbol: "BAT-T",
      decimals: 18,
      url: "https://www.basicattentiontoken.org/",
      githubHint: "",
    },
    {
      address: "0xF53E3B6c12f8c66324A64F31277260C06D869732",
      name: "Bitcoin Token",
      symbol: "BTC-T",
      decimals: 8,
      url: "https://bitcoin.org/en/",
      githubHint: "",
    },
    {
      address: "0x3b7c7C457D3aAe04a4631E4888AeEEDd08B24e41",
      name: "Digix Dao Token",
      symbol: "DGD-T",
      decimals: 9,
      url: "https://dgx.io/",
      githubHint: "",
    },
    /* Currently not in Universe (at: 0xf9d242dab5047851caac47bf5f05c2786d6f64a9)
    {
      address: '0x804b7F797eEe4D51Fef29b3Ef7E525A3848A0c0F',
      name: 'Digix Gold Token',
      symbol: 'DGX-T',
      decimals: 9,
      url: 'https://dgx.io/',
      githubHint: '',
    },
    */
    {
      address: "0xC37cdFb70BD68f6FCF2aB0a97E1D6a12eaA9215f",
      name: "Dogecoin Token",
      symbol: "DOGE-T",
      decimals: 8,
      url: "http://dogecoin.com/",
      githubHint: "",
    },
    {
      address: "0x334559E91238f466C95Bb8241555f6AD27f5978B",
      name: "Ether Classic Token",
      symbol: "ETC-T",
      decimals: 18,
      url: "https://ethereumclassic.github.io/",
      githubHint: "",
    },
    {
      address: "0x1a825E9bF3BdC8ef8B975F97c78b5208a947d0EC",
      name: "Ether Token",
      symbol: "ETH-T",
      decimals: 18,
      url: "https://ethereum.org/",
      githubHint: "",
    },
    {
      address: "0xCAAC95AB4D30EE8D6162e55EB3430134FEc5aF50",
      name: "Euro Token",
      symbol: "EUR-T",
      decimals: 8,
      url: "https://www.decentralizedcapital.com/#!/",
      githubHint: "",
    },
    {
      address: "0xC73D78870bA5a3eAddC2BE371Af09B2c429CB2CA",
      name: "Gnosis Token",
      symbol: "GNO-T",
      decimals: 18,
      url: "https://gnosis.pm/",
      githubHint: "",
    },
    {
      address: "0x08C24283F0b6C07ff9793a1B8534A49B32C07e73",
      name: "Golem Network Token",
      symbol: "GNT-T",
      decimals: 18,
      url: "https://golem.network/",
      githubHint: "",
    },
    {
      address: "0xc87BAD39aaDB70257E2417Bd8f4983361599394D",
      name: "Iconomi Token",
      symbol: "ICN-T",
      decimals: 18,
      url: "https://www.iconomi.net/",
      githubHint: "",
    },
    {
      address: "0xF051264ab9046fd73cBd00df5E732d2cA78ee704",
      name: "Litecoin Token",
      symbol: "LTC-T",
      decimals: 8,
      url: "https://litecoin.com/",
      githubHint: "",
    },
    {
      address: "0x2a20ff70596e431ab26C2365acab1b988DA8eCCF",
      name: "Melon Token",
      symbol: "MLN-T",
      decimals: 18,
      url: "https://melon.network/",
      githubHint: "",
    },
    /* Currently not in Universe (at: 0xf9d242dab5047851caac47bf5f05c2786d6f64a9)
    {
      address: '0xe1b25BCaE898ab228A13EAC49EBBA8D3df9add70',
      name: 'MakerDAO Token',
      symbol: 'MKR-T',
      decimals: 18,
      url: 'https://makerdao.com/',
      githubHint: '',
    },
    */
    {
      address: "0x64aF87A36a407732320c4dc1852dEBC60cd81c5E",
      name: "Rep Token",
      symbol: "REP-T",
      decimals: 18,
      url: "https://augur.net/",
      githubHint: "",
    },
    {
      address: "0x3b43249De8Eee169Eea7226a48699FcBA8DF3686",
      name: "Ripple Token",
      symbol: "XRP-T",
      decimals: 6,
      url: "https://ripple.com/",
      githubHint: "",
    },
    {
      address: "0x4C4e2b285e446fb1974dcC3a665CAba7C189A96f",
      name: "SingularDTV Token",
      symbol: "SNGLS-T",
      decimals: 0,
      url: "https://singulardtv.com/",
      githubHint: "",
    },
    {
      address: "0x64C4406C58C512f326d83065a72F12884105520b",
      name: "Status Network Token",
      symbol: "SNT-T",
      decimals: 18,
      url: "https://status.im/",
      githubHint: "",
    },
    /* Currently not in Universe (at: 0xf9d242dab5047851caac47bf5f05c2786d6f64a9)
    {
      address: '0xbb0449e9b66e2E1438522645caE6Cf9CD8595793',
      name: '0x Protocol Token',
      symbol: 'ZRX-T',
      decimals: 18,
      url: 'https://0xproject.com/',
      githubHint: '',
    },
    */
  ],
};

export const getTokenInfoBySymbol = sym =>
  tokenInfo.kovan.find(t => t.symbol === sym.toUpperCase()) ||
  (() => {
    throw new Error(`No token found with symbol ${sym}`);
  })();
export const getTokenInfoByAddress = address =>
  tokenInfo.kovan.find(
    t => t.address.toLowerCase() === address.toLowerCase(),
  ) ||
  (() => {
    throw new Error(`No token found with address ${address}`);
  })();

export const getPrecisionBySymbol = sym => getTokenInfoBySymbol(sym).decimals;
export const getPrecisionByAddress = address =>
  getTokenInfoByAddress(address).decimals;
export const getAddress = sym =>
  getTokenInfoBySymbol(sym).address.toLowerCase();
export const getSymbol = address => getTokenInfoByAddress(address).symbol;

export default tokenInfo;
