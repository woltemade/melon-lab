...
const getConfig = async (force = false) => {
  if (force || !config) {
    ...
    UniverseContract.setProvider(web3.currentProvider);
    ...
    config.universeAddress =
      UniverseJSON.networks[web3.version.network].address;
    ...
    config.assets = assets.map(asset => ({
      ...
      ) || console.warn("no token info found for", asset.address)),
      ...asset,
    }));
    ...
    tokenInfo.kovan.forEach(
      info =>
        assets.find(
          ...
          ? null
          : console.warn("token not defined in universe", info),
    );
  }
